...
def build_portfolio_prompt(pi: PortfolioInput, cm: ComputedMetrics, news_items: Optional[List[Dict]] = None) -> str:
    ...
    instruction = (
        ...
        "3. \"News\": (Array of Strings) Return up to three items on the most material news affecting holdings in this portfolio. "
        "Each item MUST describe a specific event or action from the supplied news (e.g., announced, reported, signed a contract, raised/lowered guidance, increased/decreased pricing, raised/lowered capex or opex). "
        "Do not write generic commentary, predictions, or column labels (e.g., 'back on the Tech Corner', 'expected to continue their upward trajectory'). "
        "Write each item as a single, self-contained sentence in sentence case that names the holding (name or ticker only; no price or percent), states what happened, ties it to one clear financial mechanism (e.g., revenue/pricing/ARPU/margins/backlog/capex/opex/FCF/guidance/unit growth/mix), and states the directional impact. "
        "End the sentence with exactly ' (+)' for positive or ' (-)' for negative; place the tag only at the very end and do not add a period after it. "
        "Do not copy or lightly edit headlines. Do not use arrows (→) or telegraphic fragments.\n"
        ...
        " - Omit any item that cannot be tied to a financial mechanism or lacks a concrete event; fewer than three items is acceptable.\n"
        " - If multiple items concern the same holding, include only the most material one (respect the three-item cap).\n"
        " - Do not include metrics unless they were explicitly provided in the prompt. Do not append price or percentage after the ticker.\n"
        ...
        " - Polarity check: if the mechanism increases costs or reduces revenue/margins/FCF/guidance, use ' (-)'; if it increases revenue/ARPU/margins/backlog/FCF/guidance or reduces costs, use ' (+)'.\n\n"
