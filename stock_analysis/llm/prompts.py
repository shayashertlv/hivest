...
def build_stock_prompt(ctx: Dict[str, Any]) -> str:
    ...
    instruction = (
        ...
        "  * quarterlyReport.bottomLine = earnings.bottomLine ('beat'/'miss'/'in line'). in addition add a +/- if its positive or negative respectively. \n"
        ...
        "    - Second item (if date exists): \"Next earnings on <YYYY-MM-DD>\"\n"
        ...
        "- CRITICAL: NEVER use metric terminology. Forbidden terms include: 'beta', 'sharpe ratio', 'sortino', 'alpha', 'volatility', "
        "  'standard deviation', 'correlation', 'R-squared', 'drawdown', 'VaR', etc.\n"
        "- Convert ALL metrics to plain language conclusions that a non-technical reader can understand.\n"
        "- Create a balanced assessment covering BOTH risks AND opportunities in 2-3 concise bullets.\n"
        "- Only include points that are directly supported by the provided DATA.\n"
        "- If no meaningful risks/opportunities can be derived from DATA, OMIT THIS FIELD ENTIRELY.\n\n"
        ...
        "{\n"
        "  \"aiDailyBrief\": \"<1–3 sentence snapshot for today/yesterday, derived strictly from provided price/action context> do not use terms like rsi, beta, etc....., if one of this metrics indicates something relevant, please write just the conclusion which derives from it and not the metrics itself!)\",\n"
        "  \"sentimentDial\": {\n"
        ...
        "      ]\n"
        "    },\n"
        "    \"newsSummary\": \"<bullet list of MATERIAL news events only; each bullet should be one short, concise sentence (max one sentence per event). "
        "EXCLUDE: conference participation announcements, routine investor presentations, generic executive statements, scheduled shareholder meetings. "
        "INCLUDE ONLY: financial results/guidance, product launches/approvals, major deals/partnerships/contracts, regulatory approvals/setbacks, "
        "leadership changes with strategic impact, capital raises/financing, legal/regulatory actions, M&A activity, operational milestones with business impact.>\"\n"
        "after writing this part please review, if you repeating ideas (different points talking about the same thing) please remove them!,\n"
        "    \"risksAndOpportunities\": \"<2-3 balanced bullets covering BOTH risks AND opportunities in plain language; NO metric terminology; "
        "only include points directly supported by DATA; OMIT THIS FIELD ENTIRELY if no meaningful points can be derived>\"\n"
        "  }\n"
        "}\n\n"

        "FINAL VALIDATION CHECKLIST (BEFORE YOU OUTPUT):\n"
        "- [ ] CRITICAL: risksAndOpportunities (if present) contains ZERO metric terminology (no 'beta', 'sharpe ratio', 'volatility', 'sortino', "
        "      'alpha', 'standard deviation', 'correlation', 'drawdown', etc.). All descriptions are in plain non-technical language.\n"
        "- [ ] risksAndOpportunities contains NO numeric values or percentages—only qualitative conclusions.\n"
        "- [ ] risksAndOpportunities has 2-3 bullets that cover BOTH risks AND opportunities in a balanced way.\n"
