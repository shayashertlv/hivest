...
def build_stock_prompt(ctx: Dict[str, Any]) -> str:
    ...
    instruction = (
        ...
        "- bottomLine = one of \"beat\", \"miss\", \"in line\" based on EPS performance. \n"
        ...
        "  • Revenue Performance (if actualRevenue exists): \"Revenue of $<X>\" followed by:\n"
        "    - If revenueGrowthYoY exists: add \" (+X.X% YoY)\" or \" (-X.X% YoY)\"\n"
        "    - If both actualRevenue and estimatedRevenue exist: add \", beat estimates of $<Y> by Z%\" or \", missed estimates of $<Y> by Z%\" where Z is calculated as ((actual - estimate) / estimate) * 100, rounded to the nearest whole number. Use \"beat...by\" if actual > estimate, \"missed...by\" if actual < estimate, or omit the percentage if equal.\n"
        "    - Format revenue with adaptive units: Use \"K\" suffix for thousands (values < $1M), \"M\" suffix for millions (values < $1B), \"B\" suffix for billions (values ≥ $1B). Format with 1-2 decimals (e.g., \"$5.2B\", \"$100K\", \"$2.5M\").\n"
        ...
        "  • Next Quarter Revenue Estimate (if nextQuarterEstimatedRevenue exists): \"Next quarter estimated revenue: $<estimate>\" with adaptive units (\"K\" for < $1M, \"M\" for < $1B, \"B\" for ≥ $1B), formatted with 1-2 decimals.\n"
        "\n"
        "IMPORTANT RULES:\n"
        ...
        "- If news exists in DATA, produce 1–3 short, asset-specific bullets that are actually interesting and clearly about the asset.\n"
        "- PRIORITIZATION RULES (apply in order):\n"
        "  1) HIGHEST PRIORITY: Highly material news - earnings releases, product launches, M&A, regulatory approvals/denials, executive changes, lawsuits, partnerships, guidance updates, significant contracts, or major corporate actions directly involving the asset.\n"
        "  2) MEDIUM PRIORITY: Moderately material news - analyst upgrades/downgrades, price target changes, sector developments with clear asset implications, significant insider trading, meaningful partnerships or collaborations.\n"
        "  3) LOW PRIORITY (use ONLY if nothing higher priority exists): General mentions, watchlist inclusions, broad market commentary that references the asset, or thematic analysis pieces.\n"
        "  4) EXCLUDE ENTIRELY: Pure duplicate topics (see below), completely generic macro news with no asset connection.\n"
        "- **CRITICAL: NO DUPLICATE TOPICS** - Each bullet must cover a DISTINCT topic or event. If multiple news items discuss the same event/topic (e.g., same earnings report, same acquisition, same product launch), consolidate into ONE bullet. Never create multiple bullets about the same subject matter.\n"
        "- When in doubt about whether news qualifies, INCLUDE it rather than exclude (prioritize higher-material items first).\n"
        ...
        "- Each string must end with \"(+)\" if the bullet point is positive for the asset, \"(−)\" if negative, or no symbol if neutral.\n"
        "- Use tight, informative phrasing with a clear action/outcome (\"wins contract…\", \"guidance cut…\", \"launches product…\", \"regulatory inquiry…\"). No sources/citations, no fluff.\n"
        "- If DATA contains absolutely no news, output an empty array [].\n\n"
        "=== FORMATTING RULES ===\n"
